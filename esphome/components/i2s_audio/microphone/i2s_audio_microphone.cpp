...
// Upstream runs the capture task at 23 -- the same priority as the Wi-Fi driver
// task -- and unpinned, so it round-robins with Wi-Fi on core 0 and starves
// RX/TX in bursts, stalling TCP. A continuous line-in stream needs capture below
// the network stack, like the bare-metal firmware (priority 3, pinned to core 1).
static const ssize_t TASK_PRIORITY = 3;
  ...
  switch (this->state_) {
    case microphone::STATE_STARTING:
      ...
      if (this->task_handle_ == nullptr) {
        xTaskCreatePinnedToCore(I2SAudioMicrophone::mic_task, "mic_task", TASK_STACK_SIZE, (void *) this, TASK_PRIORITY,
