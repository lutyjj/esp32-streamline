...
host. Cleartext uses port `39000`; authenticated TLS 1.3 uses port `39001`. The
bridge serves live audio over HTTP `8088`.
...
1. Keep `cleartext_enabled` on during migration.
2. Turn on `tls_enabled`, leave `tls_port` at `39001`, and set a private
   `transport_api_token` of at least 16 characters.
3. Restart the add-on and open **Open Web UI**.
4. Use its **PCM transport** section to provision the one-time key id and PSK
   generated by the device console.
5. Verify and activate encryption from the device console.

After every device reports an authenticated source and passes a reboot and
playback check, turn off `cleartext_enabled` and restart the add-on. The
[PCM transport workflow](../docs/tcp-transport.md#enable-encryption) covers
rotation, rollback, recovery, API equivalents, and safe key retirement.
