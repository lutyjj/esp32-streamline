...
transport. The current firmware carries it over a persistent TCP connection
(raw lwIP sockets); earlier builds used one UDP datagram per frame. See
`docs/tcp-idf-transport-plan.md` for why TCP replaced UDP.
