...
host. Port `39000` accepts cleartext or authenticated TLS 1.3, according to the
add-on's `tls_enabled` setting. The two modes are mutually exclusive. The bridge
serves live audio over HTTP `8088`.
...
1. Generate the one-time bridge credential in the device's **Stream target**
   card and copy it.
2. Turn on `tls_enabled`, set a private `transport_api_token` of at least 16
   characters, and restart the add-on. Cleartext stops immediately.
3. Open **Open Web UI** and use its **PCM transport** section to provision the
   one-time key id and PSK
