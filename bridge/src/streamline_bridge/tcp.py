...
class TcpIngestServer:
    ...
    def __init__(
        self, sources: SourceRegistry[AudioPipeline], bind: str, port: int, idle_timeout_seconds: float
    ) -> None:
    ...
    def accept(self, conn: socket.socket, addr: tuple[str, int]) -> None:
        ...
        threading.Thread(
            target=receive_source, args=(self._sources, source, generation, conn, addr), daemon=True
        ).start()
