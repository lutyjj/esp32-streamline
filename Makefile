...
HA_INTEGRATION_VERSION := $(shell sed -n 's/^[[:space:]]*"version": "\([^"]*\)".*/\1/p' custom_components/streamline/manifest.json)
HA_INTEGRATION_PROJECT_VERSION := $(call toml_version,ha-integration/pyproject.toml)
...
ha-integration-check: ha-integration-lock-check ha-integration-generate-check ha-integration-lint ha-integration-test ha-integration-hassfest ha-integration-package ;
...
version-check:
	...
	@test "$(VERSION)" = "$(HA_INTEGRATION_PROJECT_VERSION)" || (echo "VERSION=$(VERSION) does not match ha-integration/pyproject.toml ($(HA_INTEGRATION_PROJECT_VERSION))" >&2; exit 2)
...
# A release version changes the Python package metadata that uv records in the
# bridge and Home Assistant integration lockfiles. Cargo.lock carries the
# firmware package version directly.
release-lock-check: version-check bridge-lock-check firmware-lock-check ha-integration-lock-check ;
...
release-check: lint bridge-test console-test firmware-test ha-addon-test ha-integration-test ha-integration-hassfest ha-integration-package
